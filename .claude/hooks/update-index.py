...
class ProjectIndexer:
    ...
    def _generate_project_tree(self) -> str:
        ...
        def list_dir(path: Path) -> Tuple[List[Path], List[Path]]:
            try:
                items = [
                    p for p in sorted(path.iterdir()) if not self._should_ignore(p)
                ]
            except Exception:
                items = []
    ...
    def _collect_locations(self) -> Dict[str, str]:
        ...
        for rel in candidates:
            ...
            if p.exists():
                desc = (
                    self._dir_description(rel)
                    if p.is_dir()
                    else self._get_file_description(p)
                )
                locations[rel] = desc
        return locations
    ...
    def update_index(self) -> Dict[str, Any]:
        ...
        updated_index["project_statistics"] = {
            ...
            "main_directories": [
                d for d in self.whitelisted_top_dirs if (self.project_path / d).exists()
            ],
        }
