...
class TestRoomSearchIntegration:
    ...
    async def test_room_search_command_with_valid_room_success(
        ...
        with patch('src.bot.handlers.room_search_handlers.get_search_service') as mock_get_service:
            mock_service = Mock()
            ...
            mock_service.search_by_room_formatted.assert_called_once_with("201", language="ru")
    ...
    @pytest.mark.asyncio
    async def test_room_search_with_invalid_room_number(
        self, mock_update_and_context
    ):
    ...
    async def test_room_search_with_no_participants_found(
        ...
        with patch('src.bot.handlers.room_search_handlers.get_search_service') as mock_get_service:
            ...
            mock_service.search_by_room_formatted.assert_called_once_with("999", language="ru")
    ...
    @pytest.mark.asyncio
    async def test_room_search_with_api_error_handling(
        self, mock_update_and_context
    ):
    ...
    async def test_room_search_performance_under_3_seconds(
        ...
        with patch('src.bot.handlers.room_search_handlers.get_search_service') as mock_get_service:
            ...
            assert execution_time < 3.0, f"Room search took {execution_time:.2f}s, exceeding 3s limit"
    ...
    async def test_room_search_alphanumeric_room_number_support(
        ...
        with patch('src.bot.handlers.room_search_handlers.get_search_service') as mock_get_service:
            mock_service = Mock()
            mock_service.search_by_room = AsyncMock(return_value=alphanumeric_participants)
            mock_service.search_by_room_formatted = AsyncMock(return_value=[
                "1. Алексей Кузнецов (Alexey Kuznetsov) - Alex"
            ])
            mock_get_service.return_value = mock_service
            ...
            mock_service.search_by_room_formatted.assert_called_once_with("A201", language="ru")
