...
class TestAirtableSchemaValidation:
    ...
    async def test_repository_uses_correct_field_ids_for_room_search(
        ...
        room_records = [record for record in sample_airtable_records_with_fields 
                       if record["fields"].get("RoomNumber") == "201"]
        repository.client.search_by_field = AsyncMock(
    ...
    async def test_repository_uses_correct_field_ids_for_floor_search(
        ...
        floor_records = [record for record in sample_airtable_records_with_fields 
    ...
    async def test_field_id_validation_for_write_operations(self, repository):
        ...
        repository.client.update_record = AsyncMock(return_value={
            "id": "rec001",
            "fields": {
    ...
    @pytest.mark.asyncio
    async def test_field_mapping_handles_missing_fields_gracefully(
        self, repository
    ):
        ...
        result = await repository.find_by_room_number("999")  # Won't find anything but shouldn't error
    ...
    def test_field_id_format_validation(self):
        ...
        for field_name, field_id in field_ids.items():
            ...
            assert field_id.startswith("fld"), f"Field ID for {field_name} doesn't start with 'fld'"
            assert len(field_id) == 17, f"Field ID for {field_name} is not 17 characters"
    ...
    async def test_field_mapping_bidirectional_consistency(
        ...
        # Update accommodation using field IDs
        await repository.update_by_id(
            original_participant.record_id, {
                "fldlzG1sVg01hsy2g": 4,      # Floor field ID
                "fldJTPjo8AHQaADVu": "401",  # RoomNumber field ID
            }
        )
