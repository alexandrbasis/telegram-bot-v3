...
class TestSearchToEditFlow:
    ...
    async def test_complete_edit_and_save_workflow(
        self, mock_get_repo, mock_update, mock_context, sample_participant
    ):
        ...
        mock_repo.update_by_id.assert_called_once_with(
            "rec123456789012345", {"role": Role.TEAM, "payment_amount": 1500}
        )
    ...
    async def test_save_with_error_and_retry_workflow(
        ...
        retry_found = any(
            any("повтор" in btn.text.lower() for btn in row) for row in buttons
        )
