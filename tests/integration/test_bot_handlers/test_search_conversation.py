...
class TestSearchConversationFlow:
    ...
    async def test_conversation_start_to_search_flow(
        ...
        with (
            ...
            mock_repo.search_by_name_enhanced.return_value = [
                (
                    ...
                    0.9,
                    "Test User",
                )  # (participant, score, formatted_result)
            ]
            ...
            assert isinstance(
                first_call.kwargs.get("reply_markup"), InlineKeyboardMarkup
            )
            # Second call should contain the ReplyKeyboardMarkup navigation keyboard
            assert isinstance(
                second_call.kwargs.get("reply_markup"), ReplyKeyboardMarkup
            )

    async def test_conversation_results_to_main_menu_flow(
