...
class TestTimeoutRecoveryIntegration:
    ...
    async def test_main_menu_text_button_re_entry_after_timeout(
        ...
        for entry_point in entry_points:
            ...
            if (hasattr(entry_point, 'filters') and 
                hasattr(entry_point.filters, 'pattern')):
                # The pattern has escaped characters, so we need to check the match differently
                pattern = entry_point.filters.pattern.pattern
                if "Главное" in pattern and "меню" in pattern:  # Check for both key parts
                    main_menu_entry_found = True
                    break
        
        assert main_menu_entry_found, "Main Menu text button should be registered as entry point"
        
        # Simulate pressing Main Menu text button (this would be after timeout)
        # The handler should process this as a new conversation start
        result = await conversation_handler.check_update(mock_update_main_menu_text)
        assert result is not None, "Main Menu text button should trigger conversation entry"
    ...
    async def test_search_text_button_re_entry_after_timeout(
        ...
        for entry_point in entry_points:
            ...
            if (hasattr(entry_point, 'filters') and 
                hasattr(entry_point.filters, 'pattern') and
                "🔍 Поиск участников" in entry_point.filters.pattern.pattern):
                search_entry_found = True
                break
        
        assert search_entry_found, "Search text button should be registered as entry point"
        
        # Simulate pressing Search text button (this would be after timeout)
        result = await conversation_handler.check_update(mock_update_search_text)
        assert result is not None, "Search text button should trigger conversation entry"
    ...
    async def test_entry_points_include_required_handlers(self):
        ...
        assert len(entry_points) >= 6, f"Expected at least 6 entry points, got {len(entry_points)}"
        
        # Check that we have the expected types of entry points
        command_handlers = [ep for ep in entry_points if hasattr(ep, 'command')]
        message_handlers = [ep for ep in entry_points if hasattr(ep, 'filters') and hasattr(ep.filters, 'pattern')]
        callback_query_handlers = [ep for ep in entry_points if hasattr(ep, 'pattern') and not hasattr(ep, 'command')]
        
        # Should have 3 command handlers (/start, /search_room, /search_floor)
        assert len(command_handlers) == 3, f"Expected 3 command handlers, got {len(command_handlers)}"
        
        # Should have 2 message handlers (Main Menu, Search)
        assert len(message_handlers) == 2, f"Expected 2 message handlers, got {len(message_handlers)}"
        
        # Should have 1 callback query handler (search)
        assert len(callback_query_handlers) == 1, f"Expected 1 callback query handler, got {len(callback_query_handlers)}"
    ...
    async def test_main_menu_entry_point_leads_to_correct_state(
        ...
        result = await main_menu_handler.callback(mock_update_main_menu_text, mock_context)
