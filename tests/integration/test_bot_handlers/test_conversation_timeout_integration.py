...
class TestConversationTimeoutIntegration:
    ...
    async def test_timeout_from_main_menu_state(
        ...
        result = await timeout_handler.callback(
            mock_update, mock_context_with_job_queue
        )
    ...
    async def test_timeout_from_room_search_state(
        self, conversation_handler, mock_update, mock_context_with_job_queue
    ):
    ...
    async def test_timeout_state_cleanup(
        ...
        mock_context_with_job_queue.user_data = {
            "search_query": "test",
            "current_participant": {"id": "123"},
        }
        mock_context_with_job_queue.chat_data = {
    ...
    async def test_timeout_with_keyboard_recovery(
        self, conversation_handler, mock_update, mock_context_with_job_queue
    ):
    ...
    def test_timeout_state_coverage(self, conversation_handler):
        ...
        for state in expected_states:
            assert (
                state in conversation_handler.states
            ), f"State {state} missing from conversation handler"

    async def test_timeout_error_recovery(
        self, conversation_handler, mock_context_with_job_queue
    ):
    ...
    async def test_timeout_message_content_integration(
        self, conversation_handler, mock_update, mock_context_with_job_queue
    ):
