...
class TestSecurityBypassAttempts:
    ...
    def mock_audit_service(self):
        ...
        def create_auth_event(*args, **kwargs):
            return AuthorizationEvent(
                ...
                action=kwargs.get('action', args[1] if len(args) > 1 else 'bypass_attempt'),
                result=kwargs.get('result', args[2] if len(args) > 2 else 'denied'),
                user_role=kwargs.get('user_role', args[3] if len(args) > 3 else None),
                cache_state=kwargs.get('cache_state', args[4] if len(args) > 4 else 'invalid'),
    ...
    async def test_user_id_injection_attempts(
        ...
        for malicious_id in malicious_user_ids:
            ...
            assert result is False, f"Malicious user ID '{malicious_id}' was not properly rejected"

            # Test role resolution
            role = get_user_role(malicious_id, mock_settings)
            assert role is None, f"Malicious user ID '{malicious_id}' got unauthorized role: {role}"

        # Verify audit service logged all injection attempts
        assert mock_audit_service.create_authorization_event.call_count >= len(malicious_user_ids)
        assert mock_audit_service.log_authorization_event.call_count >= len(malicious_user_ids)
    ...
    async def test_privilege_escalation_bypass_attempts(
        ...
        for user_id, expected_role, should_access_admin, should_access_coordinator in test_cases:
            ...
            with patch("src.utils.access_control.get_settings", return_value=mock_settings):
                ...
                if should_access_coordinator:
                    ...
                    if not should_access_admin:  # Only check if not admin (admin has coordinator access too)
                        update.message.reply_text.assert_called()

        # Verify audit service logged authorization attempts
        assert mock_audit_service.create_authorization_event.call_count >= len(test_cases) * 2
        assert mock_audit_service.log_authorization_event.call_count >= len(test_cases) * 2
