...
class TestParticipantRepositoryInterface:
    ...
    def test_repository_has_required_abstract_methods(self):
        ...
        for method in required_methods:
            assert method in repository_methods, f"Method {method} not found in repository"
            
            # Check method is marked as abstract
            method_obj = getattr(ParticipantRepository, method)
            assert hasattr(method_obj, '__isabstractmethod__'), f"Method {method} should be abstract"
    ...
    def test_create_method_signature(self):
        ...
        assert 'participant' in annotations or len(annotations) >= 2  # self + participant + return
...
class TestConcreteImplementationRequirements:
    ...
    def test_complete_implementation_succeeds(self):
        ...
        class CompleteRepository(ParticipantRepository):
            ...
            async def create(self, participant): pass
            async def get_by_id(self, record_id): pass
            async def get_by_full_name_ru(self, full_name_ru): pass
            async def update(self, participant): pass
            async def delete(self, record_id): pass
            async def list_all(self, limit=None, offset=None): pass
            async def search_by_criteria(self, criteria): pass
            async def get_by_role(self, role): pass
            async def get_by_department(self, department): pass
            async def get_by_payment_status(self, payment_status): pass
...
class TestRepositoryMethodDocstrings:
    ...
    def test_all_methods_have_docstrings(self):
        ...
        abstract_methods = [
            'create', 'get_by_id', 'get_by_full_name_ru', 'update', 'delete',
            'list_all', 'search_by_criteria', 'get_by_role', 'get_by_department',
            'get_by_payment_status', 'count_total', 'bulk_create', 'bulk_update'
        ]
        ...
        for method_name in abstract_methods:
            ...
            assert "Returns:" in docstring, f"Method {method_name} missing Returns section"
            assert "Raises:" in docstring, f"Method {method_name} missing Raises section"
...
class TestRepositoryUsageContract:
    ...
    def test_async_method_contract(self):
        ...
        for method_name in async_methods:
            ...
            assert inspect.iscoroutinefunction(method) or hasattr(method, '__code__'), \
