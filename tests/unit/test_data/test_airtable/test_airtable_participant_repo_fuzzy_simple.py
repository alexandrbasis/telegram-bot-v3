...
class TestFuzzySearchBasicFunctionality:
    ...
    @pytest.mark.asyncio
    async def test_search_by_name_fuzzy_returns_correct_type(self, repository, sample_participants):
    ...
    @pytest.mark.asyncio
    async def test_search_by_name_fuzzy_exact_match(self, repository, sample_participants):
    ...
    @pytest.mark.asyncio
    async def test_search_by_name_fuzzy_english_match(self, repository, sample_participants):
    ...
    @pytest.mark.asyncio
    async def test_search_by_name_fuzzy_partial_match(self, repository, sample_participants):
        ...
        found_alexander = any(
            "Александр" in p.full_name_ru for p, _ in result
        )
        assert found_alexander
    
    @pytest.mark.asyncio
    async def test_search_by_name_fuzzy_russian_normalization(self, repository, sample_participants):
    ...
    @pytest.mark.asyncio
    async def test_search_by_name_fuzzy_custom_threshold(self, repository, sample_participants):
        """Test fuzzy search respects custom similarity threshold."""
        repository.list_all = AsyncMock(return_value=sample_participants)
        
        # Search with very high threshold
        high_threshold_results = await repository.search_by_name_fuzzy("Alex", threshold=0.95)
        
        # Search with lower threshold
        low_threshold_results = await repository.search_by_name_fuzzy("Alex", threshold=0.3)
    ...
    async def test_search_by_name_fuzzy_result_limit(self, repository):
        ...
        result = await repository.search_by_name_fuzzy("Александр", threshold=0.5, limit=3)
        
        # Should return at most 3 results
        assert len(result) <= 3
    
    @pytest.mark.asyncio
    async def test_search_by_name_fuzzy_results_sorted(self, repository, sample_participants):
