...
class TestDataValidator:
    ...
    def test_validate_bulk_data(self, validator, valid_participant, invalid_participant):
...
class TestBusinessRules:
    ...
    def test_payment_amount_without_status(self, validator):
        ...
        assert any("payment date should be specified" in w.lower() for w in result.warnings)
    ...
    def test_payment_status_without_amount(self, validator):
        ...
        assert result.is_valid is True
        assert len(result.warnings) > 0
        assert any("payment amount should be specified" in w.lower() for w in result.warnings)
    ...
    def test_missing_contact_information(self, validator):
        ...
        assert result.is_valid is True
        assert len(result.warnings) > 0
        assert any("contact information is recommended" in w.lower() for w in result.warnings)
    ...
    def test_no_cyrillic_in_russian_name(self, validator):
        ...
        participant = Participant(
            full_name_ru="John Smith"  # Latin only
        )
...
class TestCustomConstraints:
    ...
    def test_validate_invalid_contact(self, validator):
        ...
        assert len(result.warnings) > 0
        assert any("should be a valid email or phone" in w.lower() for w in result.warnings)
    ...
    def test_validate_location_without_separator(self, validator):
        ...
        assert result.is_valid is True  # Warning, not error
        assert len(result.warnings) > 0
        assert any("should include both country and city" in w.lower() for w in result.warnings)
