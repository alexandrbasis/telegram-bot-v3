...
class TestPerMessageFunctionality:
    ...
    def test_per_message_false_is_correct_for_mixed_handlers(self, conversation_handler):
        """
        ...
        has_command_handlers = any(
            hasattr(handler, 'commands') for handler in conversation_handler.entry_points
        )
        ...
        assert per_message_value is False, "per_message=False is correct for mixed handler types"
    ...
    async def test_callback_query_handler_works_with_per_message_false(
        ...
        for handler in main_menu_handlers:
            if hasattr(handler, 'pattern') and str(handler.pattern.pattern) == '^search$':
        ...
        with patch('src.bot.handlers.search_handlers.get_user_interaction_logger') as mock_logger:
            ...
            assert search_handler.pattern.match(callback_query.data), \
