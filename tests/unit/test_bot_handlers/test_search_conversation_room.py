...
class TestSearchConversationRoomIntegration:
    ...
    async def test_room_search_command_integration(self, mock_get_service, mock_update_room_command, mock_context):
        ...
        for entry_point in handler.entry_points:
            if hasattr(entry_point, 'commands') and "search_room" in entry_point.commands:
                room_search_entry = entry_point
                break
        
        assert room_search_entry is not None, "search_room command not found in entry points"
        
        # Execute the handler
        result = await room_search_entry.callback(mock_update_room_command, mock_context)
