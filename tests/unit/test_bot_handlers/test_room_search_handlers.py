...
class TestHandleRoomSearchCommand:
    ...
    async def test_handle_room_search_command_with_room_number(self, mock_get_service, mock_update_message, mock_context):
        ...
        mock_service.search_by_room.return_value = [
            Participant(record_id="rec123", full_name_ru="Иван Петров", room="205", floor=2)
        ]
        ...
        assert "ищу" in first_call['text'].lower() or "найдено" in first_call['text'].lower()
...
class TestProcessRoomSearch:
    ...
    @pytest.mark.asyncio
    @patch('src.bot.handlers.room_search_handlers.get_search_service')
    async def test_process_room_search_found_participants(self, mock_get_service, mock_update_message, mock_context, sample_participants):
    ...
    @pytest.mark.asyncio
    @patch('src.bot.handlers.room_search_handlers.get_search_service')
    async def test_process_room_search_no_participants(self, mock_get_service, mock_update_message, mock_context):
    ...
    @pytest.mark.asyncio
    @patch('src.bot.handlers.room_search_handlers.get_search_service')
    async def test_process_room_search_invalid_room_number(self, mock_get_service, mock_context):
        ...
        call_args = message.reply_text.call_args[1]
        assert "номер комнаты" in call_args['text'].lower()
        assert "цифры" in call_args['text'].lower() or "число" in call_args['text'].lower()
    ...
    @pytest.mark.asyncio
    @patch('src.bot.handlers.room_search_handlers.get_search_service')
    async def test_process_room_search_service_error(self, mock_get_service, mock_update_message, mock_context):
