...
class TestEditHandlerLoggingIntegration:
    ...
    async def test_handle_field_edit_selection_button_field_logging(
        self, mock_show_button, mock_get_logger, mock_update, mock_context, mock_user_logger
    ):
    ...
    async def test_handle_field_edit_selection_text_field_logging(
        self, mock_show_text, mock_get_logger, mock_update, mock_context, mock_user_logger
    ):
        ...
        mock_user_logger.log_button_click.assert_called_once_with(
            user_id=12345,
            button_data="edit_field:full_name_ru",
            username="testuser"
        )
    ...
    async def test_handle_field_edit_selection_unknown_field_logging(
        ...
        mock_user_logger.log_button_click.assert_called_once_with(
            user_id=12345,
            button_data="edit_field:unknown_field",
            username="testuser"
        )
    ...
    @patch('src.bot.handlers.edit_participant_handlers.create_participant_edit_keyboard')
    async def test_handle_button_field_selection_success_logging(
        self, mock_keyboard, mock_service, mock_get_logger, mock_update, mock_context, mock_user_logger
    ):
    ...
    async def test_show_save_confirmation_with_changes_logging(
        ...
        mock_context.user_data['editing_changes'] = {'gender': Gender.MALE, 'size': Size.M}
    ...
    async def test_retry_save_logging(
        ...
        with patch('src.bot.handlers.edit_participant_handlers.save_changes') as mock_save:
...
class TestGetUserInteractionLogger:
    ...
    def test_logger_creation_enabled(self, mock_logger_class, mock_get_settings, mock_reset):
