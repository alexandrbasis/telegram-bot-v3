...
class TestShowParticipantEditMenu:
    ...
    @pytest.mark.asyncio
    async def test_show_edit_menu_with_participant_data(
...
class TestHandleTextFieldInput:
    ...
    async def test_handle_text_input_valid_text_field(self, mock_update, mock_context):
        ...
        assert result == EditStates.FIELD_SELECTION
        assert (
            mock_context.user_data["editing_changes"]["full_name_ru"] == "Петр Петров"
        )
    ...
    @pytest.mark.asyncio
    async def test_handle_text_input_valid_number_field(
        self, mock_update, mock_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_handle_text_input_invalid_number_field(
        self, mock_update, mock_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_handle_text_input_invalid_date_field(
        self, mock_update, mock_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_text_field_success_shows_complete_participant(
        self, mock_update, mock_context
    ):
        ...
        assert (
            "CANDIDATE" in message_text or "Кандидат" in message_text
        )  # Role should be displayed
        assert (
            "Kitchen" in message_text or "Кухня" in message_text
...
class TestHandleButtonFieldSelection:
    ...
    async def test_handle_button_selection_size(self, mock_update, mock_context):
        ...
        # Add participant to context for complete display
        participant = Participant(
            record_id="rec123", full_name_ru="Иван Иванов", size=Size.L
        )
        mock_context.user_data["current_participant"] = participant
    ...
    @pytest.mark.asyncio
    async def test_handle_button_selection_invalid_value(
        self, mock_update, mock_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_role_change_team_to_candidate_clears_department(
        self, mock_update, mock_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_role_change_candidate_to_team_prompts_department(
        self, mock_update, mock_context
    ):
...
class TestSaveEnforcement:
    ...
    async def test_save_blocks_team_without_department(self, mock_update):
        ...
        context.user_data = {
            "current_participant": participant,
            "editing_changes": {"role": Role.TEAM},
        }
...
class TestSaveChanges:
    ...
    async def test_save_changes_with_changes_success(
    ...
    @pytest.mark.asyncio
    @patch("src.bot.handlers.edit_participant_handlers.get_participant_repository")
    async def test_save_changes_repository_error(
        self, mock_get_repo, mock_update, mock_context
    ):
    ...
    @pytest.mark.asyncio
    @patch("src.bot.handlers.edit_participant_handlers.get_participant_repository")
    async def test_save_success_complete_participant_display(
        self, mock_get_repo, mock_update, mock_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_button_field_success_shows_complete_participant(
        self, mock_update, mock_context
    ):
...
class TestSaveConfirmation:
    ...
    @pytest.mark.asyncio
    @patch("src.bot.handlers.edit_participant_handlers.get_participant_repository")
    async def test_save_confirmation_shows_pending_changes(
        self, mock_get_repo, mock_update, mock_context
    ):
        """Test that save confirmation shows all pending changes before save."""
        participant = Participant(
            record_id="rec123", full_name_ru="Тест Участник", role=Role.CANDIDATE
        )
        changes = {
            "full_name_ru": "Новое Имя",
            "role": Role.TEAM,
...
class TestErrorHandlingWithRetry:
    ...
    @pytest.mark.asyncio
    @patch("src.bot.handlers.edit_participant_handlers.get_participant_repository")
    async def test_save_with_retry_option_on_failure(
        self, mock_get_repo, mock_update, mock_context
    ):
        ...
        retry_found = any(
            any(
                "повтор" in btn.text.lower() or "retry" in btn.callback_data.lower()
                for btn in row
            )
            for row in buttons
        )
...
class TestPaymentFieldExclusion:
    ...
    @pytest.mark.asyncio
    async def test_payment_status_excluded_from_button_fields(
        self, mock_update, mock_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_payment_date_excluded_from_text_fields(
        self, mock_update, mock_context
    ):
    ...
    def test_button_fields_constant_excludes_payment_status(self):
        ...
        button_fields_match = re.search(
            r"BUTTON_FIELDS\s*=\s*\[(.*?)\]", content, re.DOTALL
        )
        assert button_fields_match, "BUTTON_FIELDS definition not found"

        button_fields_content = button_fields_match.group(1)
        assert (
            "payment_status" not in button_fields_content
        ), "payment_status should not be in BUTTON_FIELDS"
    ...
    def test_text_fields_constant_excludes_payment_date(self):
        ...
        text_fields_match = re.search(
            r"TEXT_FIELDS\s*=\s*\[(.*?)\]", content, re.DOTALL
        )
        assert text_fields_match, "TEXT_FIELDS definition not found"

        text_fields_content = text_fields_match.group(1)
        assert (
            "payment_date" not in text_fields_content
        ), "payment_date should not be in TEXT_FIELDS"
...
class TestEditMenuDisplay:
    ...
    @pytest.mark.asyncio
    async def test_edit_menu_excludes_payment_status_display(
        self, mock_update, mock_context
    ):
        ...
        assert (
            "💰 Статус платежа" not in message_text
        ), "Payment status should not be displayed in edit menu"
        assert (
            "Статус платежа" not in message_text
        ), "Payment status should not be displayed in edit menu"

    @pytest.mark.asyncio
    async def test_edit_menu_excludes_payment_date_display(
        self, mock_update, mock_context
    ):
        ...
        # Mock participant with payment data
        participant = Participant(
            record_id="rec123", full_name_ru="Test User", payment_date=date(2025, 1, 15)
        )
        mock_context.user_data["current_participant"] = participant
        ...
        assert (
            "📅 Дата платежа" not in message_text
        ), "Payment date should not be displayed in edit menu"
        assert (
            "Дата платежа" not in message_text
        ), "Payment date should not be displayed in edit menu"
    ...
    def test_field_labels_excludes_payment_date(self):
        ...
        field_labels_match = re.search(
            r"field_labels\s*=\s*\{(.*?)\}", content, re.DOTALL
        )
        assert field_labels_match, "field_labels definition not found"

        field_labels_content = field_labels_match.group(1)
        assert (
            "'payment_date'" not in field_labels_content
        ), "payment_date should not be in field_labels dictionary"
    ...
    def test_field_labels_uses_field_specific_icons(self):
        ...
        success_message_match = re.search(
            r"success_message\s*=\s*f[\"'](.*?)[\"']", content, re.DOTALL
        )
        ...
        assert (
            "{get_field_icon(" in success_message_format
            or "field_icon" in success_message_format
...
class TestDisplayUpdatedParticipant:
    ...
    def test_display_updated_participant_function(self):
        ...
        context = Mock()
        context.user_data = {
            "editing_changes": {"full_name_ru": "Петр Петров", "role": Role.TEAM}
        }
        ...
        assert (
            "TEAM" in result or "Команда" in result
        )  # Updated role should be in result
        assert len(result) > 0
    ...
    def test_display_updated_participant_with_no_changes(self):
        ...
        # Create mock participant
        participant = Participant(
            record_id="rec123", full_name_ru="Иван Иванов", role=Role.CANDIDATE
        )
...
class TestDisplayRegressionIssue:
    ...
    async def test_text_field_edit_with_missing_participant_context(self, mock_update):
        ...
        with patch(
            "src.bot.handlers.edit_participant_handlers.ParticipantUpdateService"
        ) as mock_service:
    ...
    @pytest.mark.asyncio
    async def test_button_field_edit_with_missing_participant_context(
        self, mock_update
    ):
        """
...
class TestComprehensiveDisplayRegressionPrevention:
    ...
    async def test_display_function_exception_handling(self, mock_update):
        ...
        participant = Participant(
            record_id="rec123", full_name_ru="Тест", full_name_en="Test"
        )
        ...
        with (
            ...
            patch(
                "src.bot.handlers.edit_participant_handlers.display_updated_participant"
            ) as mock_display,
        ):
    ...
    async def test_button_display_function_exception_handling(self, mock_update):
        ...
        # Create context WITH participant but cause display function to fail
        participant = Participant(
            record_id="rec123", full_name_ru="Тест", gender=Gender.FEMALE
        )
    ...
    async def test_save_success_shows_full_profile_message(self, mock_update):
        ...
        context.user_data = {
            "current_participant": participant,
            "editing_changes": {"full_name_ru": "Петр Петров", "church": "New Church"},
        }
