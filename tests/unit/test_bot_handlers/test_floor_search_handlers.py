...
class TestHandleFloorSearchCommand:
    ...
    async def test_handle_floor_search_command_with_floor_number(self, mock_get_service, mock_update_message, mock_context):
        """Test /search_floor command with floor number."""
        # Mock search service
        mock_service = AsyncMock()
        mock_service.search_by_floor.return_value = [
            Participant(record_id="rec123", full_name_ru="Иван Петров", room_number="201", floor=2),
            Participant(record_id="rec456", full_name_ru="Мария Сидорова", room_number="205", floor=2)
        ]
        ...
        assert "этаж" in first_call['text'].lower() or "найдено" in first_call['text'].lower()
...
class TestProcessFloorSearch:
    ...
    @pytest.mark.asyncio
    @patch('src.bot.handlers.floor_search_handlers.get_search_service')
    async def test_process_floor_search_found_participants(self, mock_get_service, mock_update_message, mock_context, sample_floor_participants):
    ...
    @pytest.mark.asyncio
    @patch('src.bot.handlers.floor_search_handlers.get_search_service')
    async def test_process_floor_search_no_participants(self, mock_get_service, mock_update_message, mock_context):
    ...
    @pytest.mark.asyncio
    @patch('src.bot.handlers.floor_search_handlers.get_search_service')
    async def test_process_floor_search_invalid_floor_number(self, mock_get_service, mock_context):
        ...
        call_args = message.reply_text.call_args[1]
        assert "номер этажа" in call_args['text'].lower()
        assert "число" in call_args['text'].lower() or "цифры" in call_args['text'].lower()
    ...
    @pytest.mark.asyncio
    @patch('src.bot.handlers.floor_search_handlers.get_search_service')
    async def test_process_floor_search_service_error(self, mock_get_service, mock_update_message, mock_context):
    ...
    @pytest.mark.asyncio
    @patch('src.bot.handlers.floor_search_handlers.get_search_service')
    async def test_process_floor_search_room_grouping(self, mock_get_service, mock_update_message, mock_context):
        """Test that floor search results are properly grouped by room."""
        # Mock participants in different rooms
        participants = [
            Participant(record_id="rec1", full_name_ru="Участник 1", room_number="201", floor=2),
            Participant(record_id="rec2", full_name_ru="Участник 2", room_number="201", floor=2),  
            Participant(record_id="rec3", full_name_ru="Участник 3", room_number="205", floor=2),
        ]
        ...
        room_201_index = next(i for i, line in enumerate(lines) if "комната 201" in line.lower())
        room_205_index = next(i for i, line in enumerate(lines) if "комната 205" in line.lower())
