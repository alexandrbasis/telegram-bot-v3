...
class TestNotificationService:
    ...
    async def test_notify_admins_without_username(self, notification_service, bot_mock):
        ...
        bot_mock.send_message.assert_any_call(
            chat_id=123456789, text="🔔 Новый запрос на доступ: User (444555666)."
        )
    ...
    async def test_notify_user_denial_sends_localized_message(
        ...
        # Act
        success = await notification_service.notify_user_decision(
            user_id=999888777, approved=False, language="ru"
        )
    ...
    async def test_notify_user_with_admin_notes(self, notification_service, bot_mock):
        ...
        bot_mock.send_message.assert_called_once_with(
            chat_id=555666777, text=expected_text
        )
    ...
    async def test_notify_max_retries_exceeded(self, notification_service, bot_mock):
        ...
        # Act
        success = await notification_service.notify_user_decision(
            user_id=321321321, approved=True, language="ru"
        )
    ...
    async def test_notify_english_localization(self, notification_service, bot_mock):
    ...
    async def test_notification_with_invalid_chat_id_handles_gracefully(
        ...
        # Act
        success = await notification_service.notify_user_decision(
            user_id=999999999, approved=True, language="ru"
        )
