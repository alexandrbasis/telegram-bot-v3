...
def validate_field_mappings(
    ...
    airtable_fields = {
        field["id"]: field
        for field in airtable_schema.get("fields", [])
    }
    
    # Validate each field mapping
    for python_name, airtable_id in AIRTABLE_FIELD_IDS.items():
        if airtable_id in airtable_fields:
            airtable_field = airtable_fields[airtable_id]
            expected_type = FIELD_TYPES.get(python_name)
...
def validate_production_schema(ci_mode: bool = False) -> bool:
    ...
    try:
        ...
        if not ci_mode:
            print(f"✅ Connected to Airtable (found {len(schema.get('fields', []))} fields)")
            print()
        ...
        # Report results
        total_fields = len(AIRTABLE_FIELD_IDS)
        ...
        if not ci_mode:
            ...
            for field in target_fields:
                if field in valid:
                    field_id = AIRTABLE_FIELD_IDS[field]
            ...
            if validation_passed:
                ...
                print("   All configured fields exist in production with correct types.")
            else:
            ...
            if validation_passed:
                ...
                print(f"FAIL: {len(missing)} missing, {len(mismatches)} type mismatches")
                if missing:
                    print(f"Missing: {', '.join(f.split()[0] for f in missing)}")
                if mismatches:
                    print(f"Mismatches: {', '.join(m.split(':')[0] for m in mismatches)}")
