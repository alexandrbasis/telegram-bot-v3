...
Implements conversation flow for floor-based participant search with room grouping,
...
async def handle_floor_search_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """
...
async def process_floor_search(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """
...
async def process_floor_search_with_input(update: Update, context: ContextTypes.DEFAULT_TYPE, floor_input: str) -> int:
    """
    ...
    try:
        ...
        if participants:
            logger.info(f"Found {len(participants)} participants on floor {floor_number} for user {user.id}")
        else:
            logger.info(f"No participants found on floor {floor_number} for user {user.id}")
