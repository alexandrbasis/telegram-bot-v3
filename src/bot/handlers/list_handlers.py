    ...
    try:
        ...
        if role == "TEAM":
            ...
            await query.edit_message_text(
                text="Неизвестный тип списка", parse_mode="Markdown"
            )
            return
...
async def handle_list_navigation(
    ...
    if action == "MAIN_MENU":
        ...
        await query.edit_message_text(
            text="Возвращение в главное меню...", parse_mode="Markdown"
        )
    elif action == "PREV":
        # Handle previous page (placeholder)
        await query.edit_message_text(
            text="Предыдущая страница...", parse_mode="Markdown"
        )
    elif action == "NEXT":
        # Handle next page (placeholder)
        await query.edit_message_text(
            text="Следующая страница...", parse_mode="Markdown"
        )
