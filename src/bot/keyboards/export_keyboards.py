...
def get_export_selection_keyboard() -> InlineKeyboardMarkup:
    ...
    keyboard = [
        ...
        [
            ...
            InlineKeyboardButton(
                "👥 Экспорт команды", callback_data=ExportCallbackData.EXPORT_TEAM
            ),
        ],
        ...
        [
            ...
            InlineKeyboardButton(
                "🎯 Экспорт ROE", callback_data=ExportCallbackData.EXPORT_ROE
            ),
        ],
        # Row 4: Navigation
        [
            InlineKeyboardButton("❌ Отмена", callback_data=ExportCallbackData.CANCEL),
        ],
    ]
...
def get_department_selection_keyboard() -> InlineKeyboardMarkup:
    ...
    for i in range(0, len(departments), 2):
        ...
        row.append(
            InlineKeyboardButton(
                dept1, callback_data=ExportCallbackData.department_callback(dept1)
            )
        )
        ...
        if i + 1 < len(departments):
            ...
            row.append(
                InlineKeyboardButton(
                    dept2, callback_data=ExportCallbackData.department_callback(dept2)
                )
            )
    ...
    navigation_row = [
        InlineKeyboardButton(
            "🔙 Назад", callback_data=ExportCallbackData.BACK_TO_EXPORT_SELECTION
        ),
        InlineKeyboardButton("❌ Отмена", callback_data=ExportCallbackData.CANCEL),
    ]
