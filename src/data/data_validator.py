...
class DataValidator:
    ...
    def validate_participant(self, participant: Participant) -> ValidationResult:
        ...
        logger.debug(
            f"Validation result: {result.is_valid}, errors: {len(result.errors)}"
        )
        return result
    ...
    def validate_field(self, airtable_field: str, value: Any) -> ValidationResult:
        ...
        is_valid, error_message = self.field_mapping.validate_field_value(
    ...
    def validate_bulk_data(
        ...
        logger.debug(
            f"Bulk validation complete: {valid_count}/{len(participants)} valid"
        )
    ...
    def _validate_business_rules(
        self, participant: Participant, result: ValidationResult
    ) -> None:
        """
        ...
        if participant.payment_amount and participant.payment_amount > 0:
            if participant.payment_date is None:
                result.add_warning(
                    "Payment date should be specified when payment amount is provided"
                )

        # Rule 3: Contact information is strongly recommended
        if not participant.contact_information:
            result.add_warning(
                "Contact information is recommended for participant communication"
            )
        ...
        if participant.full_name_en:
            ...
            if re.search(r"[а-яё]", participant.full_name_en.lower()):
                result.add_error(
                    "English name should not contain Cyrillic characters",
                    "full_name_en",
                )
    ...
    def _validate_custom_constraints(
        self, airtable_field: str, value: Any, result: ValidationResult
    ) -> None:
        """
    ...
    def _validate_contact_information(
        self, contact_info: str, result: ValidationResult
    ) -> None:
        """
        ...
        if not is_email and not is_phone and len(contact_info) > 5:
            result.add_warning(
                "Contact information should be a valid email or phone number"
            )

    def _validate_payment_date(
        self, payment_date: Union[str, date], result: ValidationResult
    ) -> None:
        """
        ...
        if isinstance(payment_date, str):
            try:
                ...
                result.add_error(
                    "Payment date must be in YYYY-MM-DD format", "PaymentDate"
                )
                return
    ...
    def _validate_location(self, location: str, result: ValidationResult) -> None:
        ...
        if not has_separator and len(location.split()) < 2:
            result.add_warning(
                "Location should include both country and city information"
            )
    ...
    def get_validation_summary(
        ...
        return {
            ...
            "invalid_record_indices": [
                i for i, result in results.items() if not result.is_valid
            ],
        }
...
def validate_participant_strict(participant: Participant) -> None:
    ...
    if not result.is_valid:
        raise ValidationError(
            f"Participant validation failed: {result.get_error_summary()}"
        )
