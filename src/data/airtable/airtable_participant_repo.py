...
class AirtableParticipantRepository(ParticipantRepository):
    ...
    async def create(self, participant: Participant) -> Participant:
        ...
        try:
            ...
            if participant.contact_information:
                existing = await self.find_by_contact_information(participant.contact_information)
                if existing:
                    raise DuplicateError(f"Participant with contact info '{participant.contact_information}' already exists")
            ...
            if e.status_code == 422:
                raise ValidationError(f"Invalid participant data: {e}", e.original_error)
            else:
                raise RepositoryError(f"Failed to create participant: {e}", e.original_error)
    ...
    async def get_by_id(self, participant_id: str) -> Optional[Participant]:
        ...
        try:
            ...
            raise RepositoryError(f"Failed to get participant {participant_id}: {e}", e.original_error)
        except Exception as e:
    ...
    async def update(self, participant: Participant) -> Participant:
        ...
        try:
            ...
            updated_record = await self.client.update_record(participant.record_id, airtable_fields)
            ...
            if e.status_code == 404:
                ...
                raise RepositoryError(f"Failed to update participant: {e}", e.original_error)
        except Exception as e:
    ...
    async def delete(self, participant_id: str) -> bool:
        ...
        try:
            ...
            if e.status_code == 404:
                ...
                raise RepositoryError(f"Failed to delete participant: {e}", e.original_error)
        except Exception as e:
    ...
    async def get_by_full_name_ru(self, full_name_ru: str) -> Optional[Participant]:
        ...
        try:
            ...
            raise RepositoryError(f"Failed to find participant by full name: {e}", e.original_error)
        except Exception as e:
            raise RepositoryError(f"Unexpected error finding participant by full name: {e}", e)
    ...
    async def list_all(self, limit: Optional[int] = None, offset: Optional[int] = None) -> List[Participant]:
        ...
        try:
            ...
            if offset is not None and offset > 0:
                logger.warning("Offset pagination not directly supported by Airtable API, offset will be ignored")
    ...
    async def search_by_criteria(self, criteria: Dict[str, Any]) -> List[Participant]:
        ...
        try:
            ...
            raise RepositoryError(f"Failed to search participants by criteria: {e}", e.original_error)
        except ValueError:
            raise
        except Exception as e:
            raise RepositoryError(f"Unexpected error searching participants by criteria: {e}", e)
    ...
    async def get_by_payment_status(self, payment_status: str) -> List[Participant]:
        ...
        try:
            ...
            logger.debug(f"Found {len(participants)} participants with payment status: {payment_status}")
            return participants
            
        except AirtableAPIError as e:
            raise RepositoryError(f"Failed to find participants by payment status: {e}", e.original_error)
        except Exception as e:
            raise RepositoryError(f"Unexpected error finding participants by payment status: {e}", e)
    ...
    async def find_by_contact_information(self, contact_info: str) -> Optional[Participant]:
        """
        ...
        try:
            ...
            raise RepositoryError(f"Failed to find participant by contact info: {e}", e.original_error)
        except Exception as e:
            raise RepositoryError(f"Unexpected error finding participant by contact info: {e}", e)
    ...
    async def find_by_telegram_id(self, telegram_id: int) -> Optional[Participant]:
        ...
        try:
            ...
            raise RepositoryError(f"Unexpected error finding participant by Telegram ID: {e}", e)
    ...
    async def search_by_name(self, name_pattern: str) -> List[Participant]:
        ...
        try:
            ...
            # Build Airtable formula for partial name matching
            # Search in both Russian and English name fields
            formula = f"OR(SEARCH('{name_pattern}', {{Full Name (RU)}}), SEARCH('{name_pattern}', {{Full Name (EN)}}))"
            ...
            logger.debug(f"Found {len(participants)} participants matching name pattern")
            return participants
            
        except AirtableAPIError as e:
            raise RepositoryError(f"Failed to search participants by name: {e}", e.original_error)
        except Exception as e:
            raise RepositoryError(f"Unexpected error searching participants by name: {e}", e)
    ...
    async def find_by_role(self, role: str) -> List[Participant]:
        ...
        try:
            ...
            raise RepositoryError(f"Failed to find participants by role: {e}", e.original_error)
        except Exception as e:
            raise RepositoryError(f"Unexpected error finding participants by role: {e}", e)
    ...
    async def find_by_department(self, department: str) -> List[Participant]:
        ...
        try:
            ...
            logger.debug(f"Found {len(participants)} participants in department: {department}")
            return participants
            
        except AirtableAPIError as e:
            raise RepositoryError(f"Failed to find participants by department: {e}", e.original_error)
        except Exception as e:
            raise RepositoryError(f"Unexpected error finding participants by department: {e}", e)
    ...
    async def bulk_create(self, participants: List[Participant]) -> List[Participant]:
        ...
        try:
            ...
            if e.status_code == 422:
                raise ValidationError(f"Invalid participant data in bulk create: {e}", e.original_error)
            else:
                raise RepositoryError(f"Failed to bulk create participants: {e}", e.original_error)
        except Exception as e:
    ...
    async def bulk_update(self, participants: List[Participant]) -> List[Participant]:
        ...
        for participant in participants:
            if not participant.record_id:
                raise ValidationError("Cannot bulk update participants without record_id")
        ...
        try:
            ...
            if e.status_code == 422:
                raise ValidationError(f"Invalid participant data in bulk update: {e}", e.original_error)
            else:
                raise RepositoryError(f"Failed to bulk update participants: {e}", e.original_error)
        except Exception as e:
            if isinstance(e, ValidationError):
                raise
    ...
    async def count_all(self) -> int:
        ...
        try:
            ...
            raise RepositoryError(f"Failed to count participants: {e}", e.original_error)
        except Exception as e:
