...
class AirtableClient:
    ...
    def __init__(self, config: AirtableConfig):
        ...
        logger.info(f"Initialized Airtable client for base {config.base_id}, table {config.table_name}")
    ...
    async def get_record(self, record_id: str) -> Optional[RecordDict]:
        ...
        try:
            ...
            record = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: self.table.get(record_id)
            )
    ...
    async def delete_record(self, record_id: str) -> bool:
        ...
        try:
            ...
            await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: self.table.delete(record_id)
            )
    ...
    async def list_records(
        ...
        try:
            ...
            records = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: list(self.table.all(**params))
            )
        ...
        for i in range(0, len(records), batch_size):
            ...
            try:
                ...
                batch_results = await asyncio.get_event_loop().run_in_executor(
                    None,
                    lambda: self.table.batch_create(batch)
                )
    ...
    async def bulk_update(self, updates: List[Dict[str, Any]]) -> List[RecordDict]:
        ...
        for i in range(0, len(updates), batch_size):
            ...
            try:
                ...
                batch_results = await asyncio.get_event_loop().run_in_executor(
                    None,
                    lambda: self.table.batch_update(batch)
                )
