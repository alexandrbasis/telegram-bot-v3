...
class AirtableScheduleRepository:
    ...
    def _build_formula(date_from: dt.date, date_to: dt.date) -> str:
        """Build Airtable formula to filter by active flag and date range."""
        # Field names follow the model serialization used across tests and utilities
        # We deliberately use plain field names here to avoid central mapping coupling
        ...
        return (
            ...
            f"IS_AFTER({{Date}}, '{start_iso}' ) + 1, "  # >= start
            f"IS_BEFORE({{Date}}, '{end_iso}' ) + 1"  # <= end
            ")"
        )
