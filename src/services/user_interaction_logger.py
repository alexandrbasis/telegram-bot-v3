...
class UserInteractionLogger:
    ...
    def __init__(self, logger_name: str = "user_interaction", log_level: int = logging.INFO):
        """
    ...
    def log_button_click(self, user_id: Optional[int], button_data: str, username: Optional[str] = None) -> None:
        """
        ...
        try:
            ...
            if not button_data:
                self._logger.warning(f"BUTTON_CLICK log attempted with empty button_data for user {user_id}")
                return
    ...
    def log_bot_response(self, user_id: int, response_type: str, content: str, 
                        keyboard_info: Optional[str] = None) -> None:
        """
    ...
    def log_missing_response(self, user_id: int, button_data: str, error_type: str, 
        ...
        try:
            ...
            self._logger.error(f"Failed to log missing response for user {user_id}: {e}")
    
    def log_journey_step(self, user_id: int, step: str, context: Optional[Dict[str, Any]] = None) -> None:
        """
        ...
        try:
            ...
            log_message = (
                f"JOURNEY_STEP [{timestamp}] "
                f"user_id={user_id} step={step}"
            )
    ...
    def log_state_change(self, user_id: int, from_state: str, to_state: str, trigger: str) -> None:
        """
